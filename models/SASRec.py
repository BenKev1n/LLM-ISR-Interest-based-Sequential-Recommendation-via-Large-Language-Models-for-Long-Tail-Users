...
class SASRec(BaseSeqModel):
    ...
    def __init__(self, user_num, item_num, device, args):
        ...
        # self.dev = device
        self.u_transfer = args.u_transfer

        self.item_emb = torch.nn.Embedding(self.item_num+2, args.hidden_size*2, padding_idx=0)
        self.pos_emb = torch.nn.Embedding(args.max_len+100, args.hidden_size*2) # TO IMPROVE
    ...
    def predict(self,
        ...
        if self.u_transfer and hasattr(model, 'W_U'):
            final_feat = model.W_U(final_feat)
        item_embs = self._get_embedding(item_indices) # (U, I, C)   #B,pos_neg,D(128)
...
class SASRec_seq(SASRec):
    ...
    def forward(self, 
        ...
        log_feats = self.log2feats(seq, positions)  #B,L,D(64)
        ...
        pos_labels, neg_labels = torch.ones(pos_logits.shape, device=self.dev), torch.zeros(neg_logits.shape, device=self.dev)
        indices = (pos != 0)    # do not calculate the padding units
        # use standard BCE losses without re-weighting
