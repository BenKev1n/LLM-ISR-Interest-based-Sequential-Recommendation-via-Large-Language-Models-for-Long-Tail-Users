...
from models.LLMISR import LLMISR_SASRec
from models.GRU4Rec import GRU4Rec
from models.SASRec import SASRec_seq
from models.Bert4Rec import Bert4Rec
from models.MELT import MELT_SASRec
...
class Trainer(object):
    ...
    def __init__(self, args, logger, writer, device, generator):
        ...
        # Global training step counter for warmup scheduling in models
        self.global_step = 0
    ...
    def _create_model(self):
        '''create your model'''
        if self.args.model_name == "llmisr_sasrec":
            self.model = LLMISR_SASRec(self.user_num, self.item_num, self.device, self.args)
        elif self.args.model_name == "llmisr_gru4rec":
            self.model = LLMISR_GRU4Rec(self.user_num, self.item_num, self.device, self.args)
        elif self.args.model_name == "llmisr_bert4rec":
            self.model = LLMISR_Bert4Rec(self.user_num, self.item_num, self.device, self.args)
        ...
        elif self.args.model_name == "melt_sasrec":
            self.model = MELT_SASRec(self.user_num, self.item_num, self.device, self.args)
    ...
    def train(self):
        ...
        # Print user group performance for best epoch
        self.logger.info("User Group Performance:")
        for key in ['Short NDCG@10','Short HR@10','Long NDCG@10','Long HR@10']:
            if key in best_res:
                self.logger.info('\t %s: %.5f' % (key, best_res[key]))
        for key in ['Tail NDCG@10','Tail HR@10','Popular NDCG@10','Popular HR@10']:
            if key in best_res:
                self.logger.info('\t %s: %.5f' % (key, best_res[key]))
